...
particleMCMC_death <- function(x, x0,
                               ...
                               target_sucess = 51L,
                               dt, ndpost, nskip,
                               filter = c("bootstrap", "alive"),
                               shape_prior = 10L, rate_prior = 1000L,
                               sd_prop = 1.0,
                               printevery = 10L) {

  filter <- match.arg(filter)
  RunFilter <- if (filter == "bootstrap") "RunBootstrapFilter" else "RunAliveFilter"
  # Draw from the prior
  ...
  # mod$RunBootstrapFilter(theta_cur)
  if (filter == "bootstrap") mod$RunBootstrapFilter(theta_cur)
  else mod$RunAliveFilter(theta_cur)
  lml_cur <- mod$lml
  ...
  for (k in seq_len(niter)) {
    ...
    # cat(theta_prop, "\n")
    if (filter == "bootstrap") mod$RunBootstrapFilter(theta_prop)
    else mod$RunAliveFilter(theta_prop)
    lml_prop <- mod$lml
